      ...
      <v-col class="text-center">
        <v-list>
          <v-list-item style="height: 66px">
            음향효과<v-spacer></v-spacer
            ><v-switch
              inset
              :input-value="soundEffect"
              @change="$store.dispatch('toggleSoundSetting')"
            ></v-switch>
          </v-list-item>
          <v-divider></v-divider>
          <v-list-item style="height: 66px">
            진동효과<v-spacer></v-spacer
            ><v-switch
              inset
              :input-value="viberateEffect"
              @change="$store.dispatch('toggleVibrateSetting')"
            ></v-switch>
          </v-list-item>
          <v-divider></v-divider>
          <v-list-item style="height: 66px">
            어플리케이션 정보<v-spacer></v-spacer
    ...
    <v-dialog v-model="warning">
      <v-card>
        ...
        <v-row class="card-contents">
          <v-col col="12" sm="12">
            <h5>
              어플리케이션의 진행상태(클리어기록, 업적기록)를<br />모두 제거하고
              초기화합니다<br />
            </h5>
          </v-col>
        </v-row>
...
export default {
  ...
  mounted() {
    if (this.$route.path == "/settings") this.$bus.$emit("checkAchievement", 6);
  },
  methods: {
    resetApplication() {

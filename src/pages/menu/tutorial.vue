    ...
    >
      <v-col cols="12">
        <v-app-bar flat color="#495689" dark>
          <v-btn @click="skipTutorial" text> SKIP </v-btn>
          <v-spacer />
  ...
  watch: {
    steps(newval) {
      ...
      if (newval === 5) {
        this.$sounds.success();
        this.$toast.loading(
          "스위치가 가위표(X)일때는 확실히 아닌곳을 X로 표시하실 수 있습니다"
        );
        this.forwardStep(10);
      }
      if (newval === 6) {
        this.$sounds.success();
        this.$toast.loading(
          "소리가 거슬리시나요? 화면 상단의 톱니바퀴 버튼을 눌러보세요"
        );
        this.forwardStep(7);
      }
      if (newval === 7) {
      ...
      if (newval === 8) {
        this.$sounds.success();
        this.$toast.loading(
          "준비되셨나요? 이제 힌트를 보면서 퍼즐을 풀어 칸을 채워보세요!"
        );
        this.forwardStep(10);
      }
    },

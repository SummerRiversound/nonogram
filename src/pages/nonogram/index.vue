<template>
  <v-container class="pa-1" fluid>
    ...
    <v-row align="center" align-content="center">
      <v-col class="text-center">
        <GameContainer
          ...
          :drawmode="drawmode"
          :stage_id="stage_id"
          @completed="gotCompleted"
          @dropLifeCount="dropLifeCount"
        ></GameContainer>
        <div class="resp_switch_wrap" :style="`margin-top:${0.1 * w}px`">
          <v-switch
            ...
            inset
            class="resp_switch"
            :style="`transform:scale(${0.003 * w})`"
            :prepend-icon="drawmode ? 'mdi-pencil' : 'mdi-close'"
          ></v-switch>
        </div>
      </v-col>
    </v-row>
    <v-dialog v-model="complete" persistent>
      <v-card>
        ...
        <v-row class="ma-0 pa-0 text-center">
          <v-col class="text-center">
            <v-btn
              color="#49a0d5"
              id="btn-to-stages"
              @click="$router.push('/stages/' + celltype)"
              >스테이지선택</v-btn
            >
          </v-col>
        </v-row>
    ...
    <v-dialog v-model="failed" persistent>
      <v-card>
        <v-row class="warning-card-header">
        ...
        <v-row class="card-contents">
          <v-col col="12" sm="12">
            <h5>
              세번이상 틀리어 클리어에 실패하였습니다<br />다시 도전할까요?
            </h5>
          </v-col>
        </v-row>
        <v-row class="ma-0 pa-0 text-center">
  ...
  watch: {
    ...
    failed(newval) {
      if (newval) {
        this.$sounds.failed();
        try {
          window.AdsMobFlutter.postMessage("reward");
        } catch (e) {
          console.log(e);
        }
      }
  ...
  methods: {
    ...
    runTimer(param) {
      ...
      let interval = setInterval(() => {
        ...
        const sec =
          (allSec % 60).toString().length == 2
            ? (allSec % 60).toString()
            : "0" + (allSec % 60).toString();
        this.timecode = `${min} : ${sec}`;
        time += 1000;
        if (this.complete || this.failed) clearInterval(interval);
      }, 1000);
    },

    ...
    <table class="game_wrapper">
      ...
      <tr>
        <td style="width: 15vw; height: 80vw">
          <SideHints
            :sideHints="sideHints"
            :sideHintsStatus="sideHintsStatus"
          ></SideHints>
        </td>
...
export default {
  ...
  methods: {
    resetAnswer(type) {
      return new Array(type).fill(0).map(() => {
    ...
    checkComplete(answer, solution) {
      const res = answer.every((row, row_i) => {
        return row.every((col, col_i) =>
          col == 1 ? true : false == solution[row_i][col_i]
        );
      });
